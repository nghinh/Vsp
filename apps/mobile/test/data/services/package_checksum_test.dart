...
void main() {
  ...
  group('PackageFileDownloader — the bytes that arrived are the bytes hashed', () {
    late Directory tmp;
    ...
    test('rejects a file whose bytes were altered in transit', () async {
      ...
      expect(corrupted.length, honest.length, reason: 'same size, so a size '
  ...
  group('PackageReadinessService — a package on disk is verified, not assumed', () {
    late Directory tmp;
    ...
    test('an intact package is ready', () async {
      ...
      expect(await readinessFor(manifestWith(entries)), PackageReadinessReason.ok);
    });
    ...
    test('one corrupted byte is a checksum mismatch', () async {
      ...
      await File('${packageDir.path}/conditions.json')
    ...
    test('a truncated file is a checksum mismatch', () async {
      ...
      await File('${packageDir.path}/tiles.pmtiles')
