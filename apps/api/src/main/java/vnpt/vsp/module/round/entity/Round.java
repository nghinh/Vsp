...
public class Round {
    ...
    /**
     * Assigned here rather than generated by Hibernate, so that a caller may
     * supply it.
     *
     * <p>A phone starts a round before it has told anyone: the round is
     * persisted on the device and played, and the server is only asked
     * afterwards. With a server-generated id the phone had no way to name the
     * round it had already started — it kept its own UUID, and every later
     * message about that round ({@code POST /rounds/{id}/complete}) was
     * addressed to an id the server had never seen and answered
     * VSP-ERR-ROUND-001. A round played out of signal could not be finished,
     * ever, and never appeared in a history that is read from the server.</p>
     *
     * <p>The default is unchanged for anyone who does not care: every
     * {@code new Round()} still comes with a fresh random UUID. What changed
     * is that it can be overwritten before saving. {@code RoundServiceImpl}
     * is the only place that does, and it checks first that the id is either
     * unused or already this golfer's own round.</p>
     */
    @Id
